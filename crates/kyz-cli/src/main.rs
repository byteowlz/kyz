...
/// Build a scrubbed copy of the current environment, removing sensitive
/// kyz vars.
///
/// Uses `vars_os` so a non-Unicode entry is filtered (not a panic) and its
/// original bytes are preserved for the child.
fn scrubbed_env() -> Vec<(std::ffi::OsString, std::ffi::OsString)> {
    std::env::vars_os()
        .filter(|(key, _)| !env_key_is_sensitive(&key.to_string_lossy()))
        .collect()
}
...
fn handle_wrap(ctx: &RuntimeContext, cmd: &WrapCommand) -> Result<()> {
    ...
    let mut child_env = scrubbed_env();

    child_env.push((
        std::ffi::OsString::from("KYZ_VAULT_PASSWORD"),
        std::ffi::OsString::from(passphrase),
    ));

    if cmd.no_read {
        child_env.push((
            std::ffi::OsString::from("KYZ_NO_READ"),
    ...
    let temp_policy = if allow_all {
        ...
        std::fs::write(tmp.path(), &policy).context("writing temp policy file")?;
        child_env.push((
            std::ffi::OsString::from("KYZ_POLICY"),
