...
pub struct AuthRequest {
    ...
    /// Client-generated pickup capability bound to this request. Required for
    /// the one-time secrets pickup so knowing the request id alone is not
    /// enough to redeem it. Never serialized: listings and status responses
    /// must not leak it.
    #[serde(default, skip_serializing)]
    pub pickup_capability: Option<String>,
}
...
pub struct CreateAuthRequest {
    ...
    pub ttl_seconds: u64,
    /// Client-generated pickup capability (random, at least 32 characters).
    /// Bound to the request and required for the one-time secrets pickup; the
    /// server never generates or returns it.
    pub pickup_capability: String,
}
...
impl AuthRequestStore {
    ...
    /// Pick up stashed secrets (one-time: removes from stash). The caller
    /// must present the pickup capability generated at request creation; a
    /// wrong or missing capability is indistinguishable from "nothing to
    /// pick up".
    ///
    /// # Errors
    ///
    /// Returns an error if the lock is poisoned.
    pub fn pickup_secrets(
        &self,
        id: &str,
        capability: &str,
    ) -> Result<Option<StashedSecrets>, String> {
        let authorized = self
            .requests
            .read()
            .map_err(|e| format!("lock poisoned: {e}"))?
            .get(id)
            .and_then(|req| req.pickup_capability.as_deref())
            .is_some_and(|stored| ct_eq(stored.as_bytes(), capability.as_bytes()));
        if !authorized {
            return Ok(None);
        }
        Ok(self
    ...
    pub fn create(&self, params: &CreateAuthRequest) -> Result<AuthRequest, String> {
        if params.pickup_capability.trim().len() < 32 {
            return Err(
                "pickup_capability is required and must be at least 32 characters of client-generated randomness"
                    .to_string(),
            );
        }
        let now = now_unix();
        ...
        let request = AuthRequest {
            ...
            denied_by: None,
            deny_reason: None,
            pickup_capability: Some(params.pickup_capability.trim().to_string()),
        };
...
fn generate_request_id() -> Result<String, String> {
    ...
    getrandom::fill(&mut bytes).map_err(|e| format!("getrandom for request id: {e}"))?;
...
/// Generate a client-side pickup capability (URL-safe base64 of 32 random
/// bytes).
///
/// The capability is generated by the requester, sent with the create
/// request, and presented on the one-time pickup; the server never returns
/// it.
///
/// # Errors
///
/// Returns an error if the OS RNG fails.
pub fn generate_pickup_capability() -> Result<String, String> {
    use base64::Engine as _;
    let mut bytes = [0u8; 32];
    getrandom::fill(&mut bytes).map_err(|e| format!("getrandom for pickup capability: {e}"))?;
    Ok(base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(bytes))
}

/// Length-independent constant-time byte comparison for capability checks.
...
mod tests {
    ...
    fn test_params() -> CreateAuthRequest {
        CreateAuthRequest {
            ...
            ttl_seconds: 60,
            pickup_capability: "0123456789abcdef0123456789abcdef".to_string(),
        }
    }
